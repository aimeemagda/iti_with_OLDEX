function [zip_train_data n_instances zip_train_classes] = convert_instances_mat_format(fname,verbose_load_instances,n_variables)
%* Reads instances from file in the .data format (Quinlan's standard).
%* Saves instances in a datafile 'fname.mat', and returns the instances 
%* and classes in matlab format - numeric array for data and cell array 
%* for classes. Each row is an instance.
...
zip_train_data = zeros(n_instances,n_variables);
zip_train_classes = cell(n_instances,1);
...
for i=1:n_instances
    ...
    for j=1:(num_cols-1)
        ...
        elseif(str2num(this_line{1,j}))         %is it numerical?
            ...
            zip_train_data(i,j)=str2num(this_line{1,j});
        elseif(ischar(this_line{1,j}))              %is it a string? 
            ...
            if(strcmp(temp,'0'))
                %insts(i,j).symbolic = 'NUMERIC';
                %insts(i,j).val = 0;
                %do nothing, zip_train_data is already initialized to zero.
            else
                %insts(i,j).symbolic = convert_whitespace_to_underscore(this_line{1,j});
            end
        else                                        %what is it??
            %fprintf('\nWarning: type of instance %d, col %d is unknown.',i,j);
            error('Error (read_instances_mat_format):unknown value type in %s, line %d, col %d is unknown.',fname,i,j);
...
%remove .data extension from filename and make a filename with .mat ext.
filestem_data = regexp(fname,'.data','split','freespacing');
mat_fname = strcat(filestem_data{1,1},'.mat');
%save the file data
save(mat_fname,'zip_train_data','zip_train_classes');
%instances = insts;
