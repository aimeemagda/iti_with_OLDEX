function tree = fast_train(tree,train_set,test_set,do_vpruning,do_performance_measuring,do_test_incrementally)
%Absorb all training instances, then restructure tree.
%author: Paul Utgoff
...
tree = increment_class_count_key(tree,1,itmp(i,1).classname,1);
tree = add_variable_info_to_tree_node(tree,itmp(i,:),1);
    ...
    %tree = ensure_best_variable(tree,1); %for fast_train, only ensure
    %after we've added all instances
